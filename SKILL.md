...
version: 1.2.0
description: 解析RSS订阅源，提取文章标题、链接、发布时间。支持本地XML文件、目录批量解析、在线RSS链接，支持同时传入多个源并合并为一个结果文件，支持为每个源指定别名，可按时间范围、正向/反向关键词和文章字数过滤。当用户需要读取RSS订阅、抓取RSS文章列表时使用此skill。
---
...
| `--keywords kw1,kw2` | 正向关键词过滤，正文匹配任一则标记 `keywordMatched: true` 并优先排序，不命中不排除 | `--keywords FPSO,海工` |
| `--exclude kw1,kw2` | 反向关键词过滤，正文匹配任一即排除 | `--exclude 招聘,广告` |
| `--min-length N` | 文章正文字数低于N则跳过 | `--min-length 200` |
